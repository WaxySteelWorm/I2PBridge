...
// This version has been updated to accept and use a session cookie.
...
class BrowserPage extends StatefulWidget {
  ...
  // --- UPDATE: Add sessionCookie parameter ---
...
class _BrowserPageState extends State<BrowserPage> {
  ...
  String _pageContent = 'Enter an I2P address and press "Go" to browse.';
  ...
  void initState() {
    ...
    if (widget.initialUrl != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _loadPage(widget.initialUrl!);
      });
    }
  }

  void _goBack() { if (_canGoBack) { setState(() { _historyIndex--; _loadPage(_history[_historyIndex], fromHistory: true); }); } }
  ...
  Future<void> _loadPage(String url, {bool fromHistory = false, bool forceRefresh = false}) async {
    String fullUrl;
    final potentialUri = Uri.tryParse(url);
    ...
    try {
      // --- UPDATE: Add session cookie to request headers ---
        ...
        children: <Widget>[
          Row(
            children: [
              ...
              Expanded( child: TextField( controller: _urlController, decoration: const InputDecoration( hintText: 'stats.i2p', border: OutlineInputBorder(), ), onSubmitted: (value) => _loadPage(value), ), ),
            ],
          ),
          const SizedBox(height: 12),
          SizedBox( width: double.infinity, child: ElevatedButton( onPressed: _isLoading ? null : () => _loadPage(_urlController.text), style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16)), child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text('Go', style: TextStyle(fontSize: 16)), ), ),
          const SizedBox(height: 20),
          Expanded( child: Container( padding: const EdgeInsets.all(12.0), width: double.infinity, decoration: BoxDecoration( color: Colors.black.withOpacity(0.2), borderRadius: BorderRadius.circular(8), ), 
            child: _isLoading
              ? Center(child: Text(_pageContent))
              : _contentType.startsWith('image/')
                  ? Center(
                      child: CachedNetworkImage(
                        imageUrl: 'http://bridge.stormycloud.org:3000/api/v1/browse?url=$currentUrl',
